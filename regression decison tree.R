...
crt1= read.csv(file.choose())
crt1
crt1=data
summary(crt1)
# Look at the average Annual_Sales()
for(i in 2:ncol(crt1))
{
  if(length(unique(crt1[,i])) <= 5)
  {
    Annual_Sales=aggregate(x=crt1$Annual_Sales,by=list(crt1[,i]),FUN=mean)
    print(colnames(crt1)[i])
    print(Annual_Sales)
    print("****************************************")
  }
}
...
rpart.plot(x = CartFullModel, type = 4,fallen.leaves = T, cex = 0.6, nn=T, tweak= 0.8)
...
windows()
fancyRpartPlot(model = CartFullModel,  cex = 0.4, tweak= 0.6)
